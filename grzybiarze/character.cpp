...
void Character::animate(const sf::Time &elapsed, char direction) {
    ...
    if(direction=='w' && rectangle_bounds.top>0 && is_in_collision_top==false){        //chodzenie strzalkami, nieprzenikanie przez krawędzie
        move(0.0, -(velocity.y*elapsed.asSeconds()));
        setTextureRect(sf::IntRect(0, 610, 150, 190));  // zmiana pozycji postaci
    }
    if(direction=='s' && rectangle_bounds.top+rectangle_bounds.height < window.getSize().y && is_in_collision_bottom==false){
        move(0.0, velocity.y*elapsed.asSeconds());
        setTextureRect(sf::IntRect(0, 0, 150, 190));
    }
    if(direction=='a' && rectangle_bounds.left > 0 && is_in_collision_left==false){
        move(-(velocity.x*elapsed.asSeconds()), 0.0);
        setTextureRect(sf::IntRect(150, 400, 150, 190));
    }
    if(direction=='d' && rectangle_bounds.left + rectangle_bounds.width < window.getSize().x&&is_in_collision_right==false){
        move(velocity.x*elapsed.asSeconds(), 0.0);
